  ...
  avg_data <- lapply(timesteps, function(z){
    ...
    period <- z
      ...
      if(period == 1995){
        ...
        y <- 8
        colnames(data)[3:length(colnames(data))] <- c(paste0(model_type, "_gfdl-esm4_ssp370_block_", seq(1:(n/y))))
        colnames(data)[1] <- "x"
        colnames(data)[2] <- "y"
      } else{
        y <- 3 
...
if(length(which(!file.exists(filename))) > 0){
  ...
  lapply(filename, function(x){
    ...
    ncatt_put(nc, varid=0, attname="institution", attval="University of Bern")
    nc_close(nc)
  })
