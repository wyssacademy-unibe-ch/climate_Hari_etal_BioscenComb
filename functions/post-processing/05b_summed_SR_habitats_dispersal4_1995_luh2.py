...
for sdm in sdms:
    ...
    for gcm in gcms:
        ...
        for habitat in habitats:
            newvalue_hist_sum[sdm][gcm][habitat] = []
            newvalue_future_sum[sdm][gcm][habitat] = []
            sumbin_hist_sum[sdm][gcm][habitat] = []
            sumbin_future_sum[sdm][gcm][habitat] = []
...
for habitat in habitats:
    for sdm in sdms:
        for taxa in taxas:
            ...
            netcdf_path_format_future = "/storage/scratch/users/ch21o450/data/LandClim_Output/Dispersal4_newbase/{}/{}/{}/{}/{}_[{}].nc"
            netcdf_path_format_hist = "/storage/scratch/users/ch21o450/data/LandClim_Output/Dispersal4/{}/{}/EWEMBI/{}_[{}].nc"
            ...
            for species_name in species_names:
                ...
                for gcm in gcms:
                    try:
                        ...
                        newvalue_hist_sum[sdm][gcm][habitat].append(newvalue_hist)
                        newvalue_future_sum[sdm][gcm][habitat].append(newvalue_future)
                        sumbin_hist_sum[sdm][gcm][habitat].append(sum_bin_hist)
                        sumbin_future_sum[sdm][gcm][habitat].append(sum_bin_future)
...
for habitat in habitats:
    for sdm in sdms:
        for gcm in gcms:
            ...
            newvalue_hist_sum_habitat = xr.concat(newvalue_hist_sum[sdm][gcm][habitat], dim="species").sum(dim="species")
            newvalue_future_sum_habitat = xr.concat(newvalue_future_sum[sdm][gcm][habitat], dim="species").sum(dim="species")
            sum_bin_hist_sum_habitat= xr.concat(sumbin_hist_sum[sdm][gcm][habitat], dim="species").sum(dim="species")
            sum_bin_future_sum_habitat = xr.concat(sumbin_future_sum[sdm][gcm][habitat], dim="species").sum(dim="species")

            # Output file paths
            newvalue_hist_sum_path = os.path.join(output_dir, f"newvalue_hist_sum_{sdm}_{gcm}_{habitat}_{scenario}_{time}_dispersal4_newbase.pkl")
            newvalue_future_sum_path = os.path.join(output_dir, f"newvalue_future_sum_{sdm}_{gcm}_{habitat}_{scenario}_{time}_dispersal4_newbase.pkl")
            sum_bin_hist_sum_path = os.path.join(output_dir, f"sum_bin_hist_sum_{sdm}_{gcm}_{habitat}_{scenario}_{time}_dispersal4_newbase.pkl")
            sum_bin_future_sum_path = os.path.join(output_dir, f"sum_bin_future_sum_{sdm}_{gcm}_{habitat}_{scenario}_{time}_dispersal4_newbase.pkl")

            # Write to pickle files
            with open(newvalue_hist_sum_path, "wb") as f:
                pickle.dump(newvalue_hist_sum_habitat, f)

            with open(newvalue_future_sum_path, "wb") as f:
                pickle.dump(newvalue_future_sum_habitat, f)

            with open(sum_bin_hist_sum_path, "wb") as f:
                pickle.dump(sum_bin_hist_sum_habitat, f)

            with open(sum_bin_future_sum_path, "wb") as f:
                pickle.dump(sum_bin_future_sum_habitat, f)
