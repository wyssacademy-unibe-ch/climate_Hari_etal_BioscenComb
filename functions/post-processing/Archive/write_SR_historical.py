...
for model in models:
    ...
    for species_name in species_names:
        for model_name in model_names:
            ds = xr.open_dataset(netcdf_path_format_hist.format(model, taxa,  species_name, historical_time), decode_times=False)
...
filename = f"/storage/scratch/users/ch21o450/data/SR/{taxa}_{model}_{historical_time}_summedprobs_newvalue.nc"
total_mean_newvalue.to_netcdf(filename)


total_mean_sum_bin = xr.concat(mean_values_sum_bin, dim="species").sum(dim="species")

filename2 = f"/storage/scratch/users/ch21o450/data/SR/{taxa}_{model}_{historical_time}_summedprobs_sum.nc"
