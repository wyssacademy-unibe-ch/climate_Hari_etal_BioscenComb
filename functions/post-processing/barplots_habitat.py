    ...
    for gcm in newvalue_dict_fut[sdm]:
        for habitat in newvalue_dict_fut[sdm][gcm]:
            if len(newvalue_dict_hist[sdm][gcm][habitat]) > 0 and len(newvalue_dict_fut[sdm][gcm][habitat]) > 0:
                ...
                climate_change_loss = climate_change
                climate_land_change_loss = climate_land_change
...
with open(os.path.join(output_dir, f"mean_newvalue_change_habitat_{scenario}_{time}.pkl"), "wb") as f:
...
with open(os.path.join(output_dir, f"mean_land_use_change_habitat{scenario}_{time}.pkl"), "wb") as f:
    pickle.dump(mean_land_use_change, f)

with open(os.path.join(output_dir, f"mean_values_habitat{scenario}_{time}.pkl"), "wb") as f:
