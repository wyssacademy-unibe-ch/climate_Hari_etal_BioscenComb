...
realm_coordinates <- read.csv(paste0(filedir, "/Realm_Coordinates/realm_coordinates.csv"))
...
climAll3 <- climAll[grepl(climAll, pattern="ssp370_2050")]
climAll4 <- climAll[grepl(climAll, pattern="ssp370_2080")]

# Remove rcp85
climAll <- climAll[!grepl(climAll, pattern="ssp370")]
...
Rneighbours <- read.csv("/storage/homefs/ch21o450/scripts/BioScen1.5_SDM/data/realm_neighbours.csv")
...
merge_some <- lapply(spPredict, function(l, ...) {
  n <- length(l)
  k <- floor(n/2)
  result <- list()
  for(i in 1:k) {
...
readr::write_csv(merge_some, paste0(predPaths[period], "/",spName,"_",pseudoabsrep,"_proj.csv.xz"))
